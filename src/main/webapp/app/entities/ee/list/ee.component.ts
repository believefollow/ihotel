...
export class EeComponent implements OnInit {
  ...
  trackId(index: number, item: IEe): number {
    return item.id!;
  }

  delete(ee: IEe): void {
    const modalRef = this.modalService.open(EeDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IEe[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
