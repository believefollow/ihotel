...
import ihotel.app.service.CheckinQueryService;
import ihotel.app.service.CheckinService;
...
/**
 * REST controller for managing {@link ihotel.app.domain.Checkin}.
 */
@RestController
@RequestMapping("/api")
public class CheckinResource {
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/checkins/{id}", consumes = "application/merge-patch+json")
     ...
     */
    @GetMapping("/checkins")
    public ResponseEntity<List<CheckinDTO>> getAllCheckins(CheckinCriteria criteria, Pageable pageable) {
    ...
    /**
     * {@code SEARCH  /_search/checkins?query=:query} : search for the checkin corresponding
     * to the query.
     *
     ...
     */
    @GetMapping("/_search/checkins")
    public ResponseEntity<List<CheckinDTO>> searchCheckins(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Checkins for query {}", query);
        Page<CheckinDTO> page = checkinService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
